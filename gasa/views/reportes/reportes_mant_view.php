...
function cambia_sucursal_codigo(codigo)
{
	


var txt = 'Elegir sucursal <select name="sucursal_id" id="Csucursal" style="size: 22.5em;">';
	<?php foreach($sucursales->result() as $row){?>
			txt += '<option value="<?php echo $row->id;?>" selected="selected"><?php echo $row->nombre;?></option>';
	<?php } ?>
	txt += '</select>';
...
$.prompt(txt,{ 
					...
					callback: function(v,m){
						var sucursal_id = m.find('#Csucursal').val();
						var sucursal = m.find('#Csucursal option:selected').text();
						if(v){
							  $.jGrowl('Producto: '+codigo+' pasado a Disponible <br/> y a la Sucursal :'+sucursal,{
											beforeOpen: function(e,m,o) {
												$.post('reportes/cambiarPaDisponibleSucursal',{codigo:codigo,sucursal_id:sucursal_id}, function(data){
															
														});
											},
											close: function(e,m,o) {
												window.location.replace("<?php echo base_url();?>"+"reportes/mantenimientos"); 
...
{
	$.jGrowl('Codigo Producto:'+codigo+' pasado a Disponible',{
											beforeOpen: function(e,m,o) {
												$.post('reportes/cambiarPaDisponible',{codigo:codigo}, function(data){
											...
											close: function(e,m,o) {
												window.location.replace("<?php echo base_url();?>"+"reportes/mantenimientos"); 
												//window.location ="reportes/mantenimientos";
...
function toma_datos_impresion()
{
	...
	$("input[name='pfecha_mante']").val($('#fecha_mante').val());
}
...
<tr>
      ...
      <tr>
        <td class="normales" >
			<form action="reportes/imprimir_mantenimiento" method="post" name="form_print" target="_blank" style="width:250px;" onsubmit="toma_datos_impresion()">
			...
			<input name="pfecha_mante" type="hidden" value="" />
  ...
  <tr>
    <td>
  <form action="reportes/mantenimientos" method="post">
    <table width="100%" border="0" cellpadding="3" cellspacing="0">
	  <tr bgcolor="#666666">
        ...
        <td width="24%" bgcolor="#666666" class="normales_bold_blanco"><div align="left">Fecha Mantenimiento </div></td>
      ...
      <tr bgcolor="#666666">
        ...
        <td width="16%" bgcolor="#666666" class="normales_bold_blanco"><div align="left"><? if($sucursal ==''){echo form_dropdown('sucursal_id', $options_suc,$user['sucursal_id'],"class='sel_sucursal' id='sucursal_id' style='size: 22.5em;width:140px;'");}else{ echo form_dropdown('sucursal_id', $options_suc,$sucursal,"class='sel_sucursal' style='size: 22.5em;width:140px;'");}?></div></td>
        <td width="24%" bgcolor="#666666" class="normales_bold_blanco"><div align="left"><?php if($fecha_m =='' ){ ?>
          <input class="date-pick" name="fecha_mante" id="fecha_mante" type="text" size="12" style="width:70px;"/>
			<?php } else{ ?>
			 <input class="date-pick" name="fecha_mante" id="fecha_mante" type="text" size="12" value="<?php echo $fecha_m;?>" style="width:70px;"/>
			<?php } ?></div>
</td>
        </tr>
</table>
	...
	<table width="96%" border="0" cellpadding="3" cellspacing="0">
      ...
      <tr class="normales_chicas">
        <td width="6%" class="normales"><div class="button"><a href="javascript:;"   style=" text-decoration:none;" onclick="viewInfo('<? echo $producto->codigo; ?>',<?php echo $producto->status;?>);"><img src="images/ver.png" alt="ver detalles" width="16" height="17" style="cursor:pointer;" border="0"/></a></div></td>
        <td width="18%" class="normales"><div align="left"><?php echo $producto->codigo?></div></td>
        <td width="22%" class="normales"><div align="left"><?php echo $producto->nombre?></div></td>
		  <td width="13%" class="normales"><div align="left"><?php echo $producto->categoria?></div></td>
        <td width="20%" class="normales"><div align="left"><?php echo $producto->sucursal?></div></td>
        <td width="13%" class="normales"><div align="left"><?php echo date("d-m-Y",strtotime($producto->f_mante_entrada));?></div></td>

        <td width="8%" class="normales"><div align="left"><?php if(($user['rol']==1)||($user['rol']==3)){?><a href="javascript:;"  onclick="cambia_sucursal_codigo('<? echo $producto->codigo; ?>');"><input type="button" value="Disponible" /></a><?php } ?> </div></td>
      </tr>
      <?php }}?>
	<tr><td>&nbsp;</td></tr>
    </table> </div>   </td>
